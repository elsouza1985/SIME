...
todosAgendamentos(dataagora);
$('#cancel').on('click',function(){
	$('#isNewUser').val("false");
});
});

function todosAgendamentos(date){
	date = new Date(date);
	var firstDay = new Date(date.getFullYear(), date.getMonth(), 1);
	
	var campos = "DISTINCT DATE_FORMAT(Data, '%Y-%m-%d') AS 'dataevento'"
	var filtro = "WHERE Data > '"+ formatDateSQL(firstDay)+ "' AND User='"+$('#txtUserID').val()+"'";
    $.ajax({
        dataType: 'json',
        url: url+'api/getData.php',
        type : 'post',
        data: {action:'custom_select', table:'Agenda', fields:campos, filter:filtro},
        success : function(data){
        	console.log(data);
        	var dados = data.data
        	var eventDates = {};
        	 for (i = 0; i < dados.length; i++) {
        		 eventDates[new Date( dados[i].dataevento )] = new Date( dados[i].dataevento );
        	 }
...
function manageData(dataatual) {
	dataatual = formatDateSQL(dataatual);
	
	$('#txtdataAgenda').val(formatar(dataatual));
	$('#dataAgenda').val(dataatual);
...
function refreshData(dataatual) {
	dataatual = formatDateSQL(dataatual);
	...
	var htmlContent = "";
		...
		$.ajax({
			...
			var htmlcontent2 = '<tr id="txtNewUserContact" style="display:none">'+
								'<td>'+
								'<input type="tel" id="txtNumeroCliente" pattern="[0-9]*"  placeholder="Contato" class="form-control" maxlength="14">'+
								'</td>'+
								'</tr>';
			...
			$.ajax({
		        ...
		        success : function(data){
					...
					'<tr>'+
					'<th colspan="2">'+
					'<label id="modal-AgendaData">'+ $('#txtdataAgenda').val()+
					'</label> '+
			...
			'<tbody>'+
				...
				'<tr>'+
					'<td colspan=2>Contato'+
					'<a class="icon" href="https://api.whatsapp.com/send?phone='+val[0].mobile+'&amp;">'+
							'<i class="fa fa-whatsapp" style="font-size: x-large; padding: 10px;"></i>'+
	...
	if(novoUsuario == "true"){
		...
		DadosNovoCliente = {
			...
			$.ajax({
				url : url+'api/getData.php',
				type : 'post',
				data: {action:'save_cliente', table:'customers', Data:DadosNovoCliente},
		        success : function(data){
		        	Dados.Cliente = data;
		        	$.ajax({
		    			url : url+'api/getData.php',
			...
			$("#txtNomeCliente").addClass("is-invalid");
			return
		}
		// $('#Agendamento-modal').modal('toggle');
	}else{
		if( $("#ddrCliente :selected").val()!= '0'){
		$.ajax({
			url : url+'api/getData.php',
			type : 'post',
			data: {action:'save_agendamento', table:'Agenda', Data:Dados},
	        success : function(data){
			 ...
			 $("#ddrCliente").addClass("is-invalid");
			 return
		}
	}
}
